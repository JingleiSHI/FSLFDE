...
__global__ void CorrelateData(int          batch_size,
                              ...
                              const float *input_b,
                              float       *output) {
  extern __shared__ char patch_data_char[];  // SHI: patch_data_char is the address of allocated shared memory (size unknown)

  float *patch_data = (float *)patch_data_char; // SHI: got the allocated address

  // First (upper left) position of kernel upper-left corner in current center
  // position of neighborhood in image 1
  int x1     = blockIdx.x * stride_1 + max_displacement;   // SHI: the x position in the padded input
  int y1     = blockIdx.y * stride_1 + max_displacement;   // SHI: the y position in the padded input
  int item   = blockIdx.z;
    ...
    for (int i = 0; i < kernel_size; i++) {
      ...
      for (int ch = ch_off; ch < in_channels; ch += (WARPS_PER_BLOCK * THREADS_PER_WARP)) {
        int idx1 = ((item * in_height_padded + y1 + j) * in_width_padded + x1 + i) *
                   in_channels + ch;  // SHI: Attention, the length of ch is 32
  ...
  __syncthreads();  // SHI: synchronization in the thread block

  __shared__ float sum[WARPS_PER_BLOCK * THREADS_PER_WARP];  // SHI: allocated shared memory (size known)

  // Compute correlation
  for (int out_channel = 0; out_channel < out_channels; out_channel++) {
    sum[ch_off] = 0;

    int s2o = (out_channel % neighborhood_grid_width - neighborhood_grid_radius) * stride_2; // SHI: This is x position relative to x1
    int s2p = (out_channel / neighborhood_grid_width - neighborhood_grid_radius) * stride_2; // SHI: This is y position relative to y1
    int x2  = x1 + s2o;
    int y2  = y1 + s2p;
...
void Correlation(const GPUDevice& device,
                 ...
                 int              stride_2,
                 float           *output) {
  dim3 totalBlocksCorr(out_width, out_height, batch_size); // SHI: the number of block in the grid
  dim3 threadsPerBlock(THREADS_PER_WARP *WARPS_PER_BLOCK); // SHI: the number of thread in each block
